# 내 하네스 — Claude Code 세션 가이드

> 이 파일은 Claude Code 가 **세션을 시작할 때 자동으로 읽습니다.**
...
1. **세션 시작** → Claude 가 이 `CLAUDE.md` 를 자동으로 읽는다.
...
- 새 메모리 만들기: Claude 에게 "새 규칙 메모리 만들어줘"라고 하면 `scripts/new_memory.py` 로 생성.
...
```
harness-starter-kit/
├─ CLAUDE.md              ← (이 파일) 세션 시작 자동 로드
├─ README.md             ← 처음 받았을 때 읽는 설치/사용법
├─ .claude/
│  ├─ settings.json      ← 훅 연결 설정
│  └─ hooks/
│     ├─ inject_memory.py   ← 관련 메모리 자동 주입(핵심)
